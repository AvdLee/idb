...
public final class FBSimulatorSet: FBiOSTargetSet {
  ...
  class func set(withConfiguration configuration: FBSimulatorControlConfiguration, deviceSet: SimDeviceSet, delegate: (any FBiOSTargetSetDelegate)?, logger: (any FBControlCoreLogger)?) throws -> FBSimulatorSet {
  ...
  /// CoreSimulator's available devices plus booted devices hidden only because a sandboxed host
  /// cannot read an Xcode 27 cryptex runtime mount. Booted devices remain fully operable through
  /// their `SimDevice` XPC handle; unavailable shutdown devices stay excluded.
  private var devicesForInflation: [Any] {
    ...
    let bootedState = UInt64(FBiOSTargetState.booted.rawValue)
    var devices = availableDevices
    for device in allDevices {
      guard
        let state = Self.deviceState(device),
        state == bootedState,
        let udid = Self.deviceUDID(device),
        !availableUDIDs.contains(udid)
      else {
        continue
      }
