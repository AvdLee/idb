...
enum DTUHIDTiming {
  ...
  static let drainNanos: UInt64 = 200_000_000 // 200ms
...
actor SimulatorDTUHIDTransport {
  ...
  /// Encodes `payload` and enqueues it on the connection. Simulator endpoint send-barrier callbacks
  /// do not fire in sandboxed clients, so delivery must not await one. `flush()` provides the bounded
  /// daemon-consumption window once per gesture rather than per primitive.
  ///
  ...
  /// Writes an already-encoded message without a send barrier. See `send(messageType:payload:)`.
