...
/// Adapts the `XC_kAXXC*` dictionaries returned by the axbridge guest to `AXPlatformElement`.
final class AXBridgePlatformElement: FBAXPlatformElement {
  private let attributes: [String: Any]
  private let childElements: [AXBridgePlatformElement]
  private let pid: pid_t

  init(attributes: [String: Any], children: [AXBridgePlatformElement], pid: pid_t) {
    self.attributes = attributes
    self.childElements = children
    self.pid = pid
  }

  static func tree(from node: [String: Any], pid: pid_t) -> AXBridgePlatformElement {
    let children = (node[AXWire.Node.children.rawValue] as? [[String: Any]] ?? [])
      .map { tree(from: $0, pid: pid) }
    return AXBridgePlatformElement(attributes: node, children: children, pid: pid)
  }

  func axFrame() -> NSRect {
    guard let raw = attributes[AXWire.Node.frame.rawValue] else { return .zero }
  ...
  func axRole() -> String? {
    ...
    let automationName = elementTypeName(AXWire.Node.automationType.rawValue)
    let identifying =
      (automationName == AXRoleVocabulary.untypedName ? nil : automationName)
      ?? elementTypeName(AXWire.Node.elementType.rawValue)
      ?? translatorSubroleName()
      ?? translatorRoleName()
      ?? className(AXWire.Node.elementType.rawValue)
    return identifying
      ?? automationName
      ?? stringAttribute(AXWire.Node.automationType.rawValue)
      ?? stringAttribute(AXWire.Node.elementType.rawValue)
  }

  func axLabel() -> String? { stringAttribute(AXWire.Node.label.rawValue) }
  func axValue() -> Any? { attributes[AXWire.Node.value.rawValue] }
  func axIdentifier() -> String? { stringAttribute(AXWire.Node.identifier.rawValue) }
  ...
  func axIsEnabled() -> Bool { boolAttribute(AXWire.Node.isEnabled.rawValue) ?? true }
  func axIsRequired() -> Bool { false }
  func axIsExpanded() -> Bool { false }
  func axIsHidden() -> Bool { false }
  func axIsFocused() -> Bool { false }

  func axIsHittable() -> Bool? { boolAttribute(AXWire.Node.isVisible.rawValue) }
  func axHittablePoint() -> CGPoint? { pointAttribute(AXWire.Node.visiblePoint.rawValue) }
  func axCentrePoint() -> CGPoint? { pointAttribute(AXWire.Node.centerPoint.rawValue) }
  func axIsUserInteractionEnabled() -> Bool? { boolAttribute(AXWire.Node.userInteractionEnabled.rawValue) }
  ...
  func axExplainedBy() -> FBAXPlatformElement? {
    guard let attributes = self.attributes[AXWire.Node.explainedBy.rawValue] as? [String: Any] else {
      return nil
    }
    return AXBridgePlatformElement(attributes: attributes, children: [], pid: pid)
  }
  ...
  private func translatorSubroleName() -> String? {
    guard let raw = (attributes[AXWire.Node.translatorSubrole.rawValue] as? NSNumber)?.intValue else {
      return nil
    }
    return AXRoleVocabulary.name(forTranslatorSubrole: raw)
  }
  ...
  private func translatorRoleName() -> String? {
    guard let raw = (attributes[AXWire.Node.translatorRole.rawValue] as? NSNumber)?.intValue else {
      return nil
    }
    return AXRoleVocabulary.name(forTranslatorRole: raw)
  }
  ...
  private func elementTypeName(_ key: String) -> String? {
    ...
    return AXRoleVocabulary.name(forElementType: number)
  }
}
